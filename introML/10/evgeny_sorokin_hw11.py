...
def one_hlayer(activation_func=sigmoid):
    np.random.seed(0)
    ...
    for j in range(20000):
        ...
        a1 = activation_func(z1)          #first layer activation
        ...
        if activation_func is sigmoid:  #TODO: HERE derivative should change
            a1_delta = a2_delta.dot(W2.T) * (a1 * (1-a1))  
        elif activation_func is tanh:
            a1_delta = a2_delta.dot(W2.T) * (1 - a1**2)
        elif activation_func is relu:
...
def two_hlayers(activation_func=sigmoid):
    np.random.seed(0)
    #download the dataset
    X, labels = sklearn.datasets.make_moons(200, noise=0.20)
    y = labels.reshape(-1, 1)
